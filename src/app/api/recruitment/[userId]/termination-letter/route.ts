...
import { createTerminationLetter } from "@/lib/recruitment/pdf";
...
export async function GET(_request: NextRequest, context: { params: Promise<{ userId: string }> }) {
  ...
  if (!profile.terminationDate) return new NextResponse("Staff is not terminated", { status: 409 });
  const bytes = await createTerminationLetter(profile);
  return new NextResponse(Buffer.from(bytes), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `inline; filename="termination-${profile.user.username}.pdf"`,
