...
function validateBody(
  ...
  // Kept for compatibility with older clients. The canonical Purchase event ID
  // is generated by the server from the persisted OMS order ID.
  const eventId = normalizeText(body.eventId) || checkoutRequestId;
  ...
  try {
    ...
    const result = await prisma.$transaction(
      async (transaction) => {
        ...
        const metaPurchaseEventId = `purchase_${databaseOrderId}`;
