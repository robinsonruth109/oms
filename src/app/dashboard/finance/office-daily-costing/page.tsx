...
import { createOfficeDailyCostAction, deleteOfficeDailyCostAction } from "../actions";
...
export default async function OfficeDailyCostingPage({ searchParams }: PageProps) {
  ...
  const [entries, priorEntries] = await Promise.all([
    prisma.financeDailyCost.findMany({
      where: { bookType: "OFFICE", costDate: { gte: range.start, lte: range.end } },
      include: {
    ...
    prisma.financeDailyCost.findMany({
      where: { bookType: "OFFICE", costDate: { lt: range.start } },
    ...
    <div className="space-y-6">
      <section className="rounded-3xl bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Office Daily Costing</h1>
            <p className="mt-1 text-sm text-slate-500">
              Separate office Joma/Khoroch cash book with its own opening balance and Cash, Bank and bKash tracking.
            </p>
          </div>
          <Link
            href={`/api/finance/office-daily-costing/pdf?date=${encodeURIComponent(date)}`}
            target="_blank"
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <div className="rounded-3xl border bg-white p-5 shadow-sm sm:p-6">
          ...
          <form action={createOfficeDailyCostAction} className="grid gap-4 sm:grid-cols-2">
          ...
          <form action={createOfficeDailyCostAction} className="grid gap-4 sm:grid-cols-2">
            ...
            <label>
              ...
              <select name="category" defaultValue="" required className="min-h-11 w-full rounded-xl border px-3 text-sm">
                ...
                {OFFICE_DAILY_EXPENSE_TYPES.map((type) => <option key={type}>{type}</option>)}
              </select>
            </label>
            <label>
              <span className="mb-1 block text-sm font-medium text-slate-700">Amount (Tk)</span>
      ...
      <section className="overflow-hidden rounded-3xl border bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b p-5 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="font-bold text-slate-900">Office Daily Cash Book</h2>
              <p className="text-sm text-slate-500">Office Joma and Khoroch for one Bangladesh business date.</p>
            </div>
          </div>
                ...
                {jomaEntries.map((entry) => (
                  <tr key={entry.id} className="border-t align-top">
                    ...
                    <td className="px-4 py-3">
                      <form action={deleteOfficeDailyCostAction}>
